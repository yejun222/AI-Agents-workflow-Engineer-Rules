...
│   └── check-config.py
├── docs/
│   ├── development-spec.md   # 全量技术规范；各角色按章节号定点引用，缺它则角色的规范引用全部悬空
│   ├── error-codes.md        # 业务错误码注册表；规范 6.4 与 CLAUDE.md 均指向它
│   ├── artifacts.md
│   ├── state-machine.md
│   ├── role-protocol.md
│   ├── config-checklist.md
...
- **60-review 的去向**：研发（按 REV 编号修复）｜ 测试执行（未闭环致命/严重项纳入 51；`已接受（用户决断）` 项除外）｜ 主对话（评审门决策）——**不流向测试设计**。**未闭环的 `REV-xx` 是发布门禁的阻断项**（`docs/state-machine.md` D2 第 4 项，**一般 / 建议级同样算**）：`/review` 阶段未闭环又未被接受的审查问题，即便覆盖达标也落 **行 4**（`不可发布` → 修复循环 a 派 code-reviewer 复核，必要时再派 engineer）—— 此前这类项不在任何判定行的条件里，登记后对结论的权重恒为 0
- **三条接受路径不要混**：
...
⚠ 修复循环 a 的其余 owner（docs/state-machine.md 修复循环节）：契约冲突 / 待确认清单 → 主对话裁决；阻断型 OBS（52 / 60）→ 按领域归属（环境 / 证据 / 工具链类由主对话调度、实现类 → engineer）；上游「待复核」→ 按 artifacts §5 失效传播矩阵重跑；未闭环 REV → code-reviewer。**无 owner 的阻断项会让行 4 在 a 处原地停滞**
```
