...
| OBS | 观察项（未立 BUG 的事实 / 契约空白 / 未审未执行项） | test-executor | **按产物命名空间限定**：`51:OBS-nn` / `52:OBS-nn` 各自独立序列；跨产物引用必须带文件名前缀（见下） |
...
- **OBS 不是缺陷的降级收纳箱**：判为 OBS 必须写明「为何不立 BUG」（无功能影响 / 契约空白 / 执行侧问题），否则会变成「把不想处理的问题扫到地毯下」。
