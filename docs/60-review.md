...
| 60:OBS-05 | 缘由：§H.3-d——`DrawService.cs` 既有注释以架构 v3 行号 `:544` / v4 `:567` 为锚点（`docs/development-spec.md` 13.4 首条实测实例）；登记时点只读实测：工作区已有未提交修复（4 处注释改为稳定引用「D-08『异常与重试』」），未提交、未复核（并入 `60:OBS-32` 统一处置）。为何不影响结论：注释级、无行为影响。 | 责任人：主对话（调度提交）；复核归 code-reviewer | 计划复核时点：2026-09-19 前（提交后随下一轮复审复核实证） | 非阻断 |
...
| 60:OBS-08 | 缘由：§H.6-2——集成测试未由本角色复跑（CHG-18 声明 43/0/0 未独立复核）。为何不影响结论：变更不触及集成断言面（`tests/integration` 对 `LogCritical` / `CRITICAL` 检索 0 命中），且单测已独立复跑 96/0/0。 | 责任人：code-reviewer | 计划复核时点：下一轮复审（v7）或集成断言面下次被触及时按矩阵复跑 | 非阻断 |
...
| 60:OBS-18 | 缘由：§G.6-5——服务端 `innodb_lock_wait_timeout` 实际值未由本角色直接读表（由守护用例动态读取并断言、实跑通过；数值不单独记录）。为何不影响结论：数值由用例动态读取并作为断言输入，不构成任何结论的要件。 | 责任人：code-reviewer | 计划复核时点：下一轮复审（v7）随集成复跑直读并登记 | 非阻断 |
...
> **未复核项台账（2026-09-18 补建，详见 §I）**：上述未完成 / 残余观察项已逐条登记为 `60:OBS-01` ~ `60:OBS-32`（非阻断 31 / **阻断 1**，逐条含缘由、责任人、计划复核时点）；其中 `60:OBS-32`（登记时点未提交、未复核、无 CHG 记录的引用修复改动集）为阻断项——按 `docs/state-machine.md` D2，其闭环前发布门禁不应按「无其他阻断项」放行；REV 统计（21 项 = 已闭环 16 / `已接受（用户决断）` 5 / 未闭环 0）不因本台账而变。
