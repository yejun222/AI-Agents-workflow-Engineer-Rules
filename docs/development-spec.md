...
| 429 | **例外：HTTP 状态码，非 `ApiResult.code` 取值** —— 限流（请求过于频繁），响应体为 `code=1001` | 提示"请稍后重试"（与 `1001` 降级路径同一处理） |
| 500 | 系统内部错误 | 通用错误提示 |
| ≥1000 | 业务异常（含业务层校验失败，如 1002） | 展示后端具体错误信息 |

> **说明**：表中数值为响应体 `ApiResult.code` 的取值。HTTP 状态码与 0–500 段保持一致（401 未登录、403 无权限、409 冲突等），**唯一例外是 `429` 限流：HTTP 429 而响应体 `code=1001`**（该行标的是 HTTP 状态码而非 `code` 取值；前端按 `code` 判定，不得另立分支）；业务异常（≥1000）HTTP 返回 200，前端依据 `code` 区分处理。
>
...
5. **架构阶段必须显式回答的六个问题**（不回答就会在下游反复付代价，且每次都要重新论证）：
   - 通用错误响应体形态（见上条）
   ...
   - **随机源可控性：随机 / 不确定性是否可注入（可否播种、可否按输入强制结果）**——不能注入则**任何「固定随机源」类前置在实现下不可构造**（实测：抽奖只有「按用户名强制结果」、无播种通道，TC-78 的「默认奖池 + 固定随机源」前置**不可满足**，只能在脚本与报告中登记为**前置偏离**）。架构阶段须就此类前置显式裁决：**补构造手段 / 改用例 / 明确不作要求**，不得留给测试去"判空白"
   - 降级与回退语义：依赖不可用（Redis 挂掉）、库存耗尽、候选集为空时分别如何表现
   - 可观测性落地：日志是否**落盘**、能否离线检索（**历史实测**：`51:OBS-05` / `52:OBS-05` 修复前，本项目 Serilog 仅 `WriteTo: Console`，离线无法检索，用例只能绕道三面交叉验证；**现行事实**：该修复已追加 Serilog `File` sink（见 `docs/40-changelog.md` CHG-21），日志已落盘、可离线检索。但**答案是逐项目的**——本项目结论只证明该问可回答，不证明答案必然为「是」，故仍须在架构阶段显式回答并登记，不得默认沿用本项目结论）
...
6. **由缺陷驱动的修复，其断言必须实测变红（负向验证）**：「写明它会在什么状态下失败」（第 2 条）只是文档义务，**不等于它真的会失败**。修复完成后必须做一次实测：**回退修复（或构造修复前的等价样本）→ 断言必须失败 → 还原 → 断言必须复绿**，三次观测（回退方式 / 观察到的失败 / 还原后结果）记入 `docs/40-changelog.md`。不具备可回退性时，须说明理由并给出**等价负向样本**。范式即本仓库的护栏自测 `python3 tools/check-config.py --self-test`（逐条突变 → 目标断言必红 → 还原 → 恢复全绿）——**写成「会失败」但从未实测过的断言，与没有断言等价**

### 7.6 用例独立性与不稳定判定

用例之间**共享可变状态**会产出「失败」但**不代表产品缺陷**，而它的表象与产品缺陷完全一致（本仓库实测：全量串行 4 条用例超时失败、`--grep` 隔离复跑**秒过**，两轮共 10 条次"失败"实为共享限流窗口所致）。三条硬性要求 + 一条判定口径：

1. **独立性**：每条用例必须**自带隔离前提**——不依赖执行顺序、不共享可变状态（库行 / Redis 键 / 出口 IP 的限流窗口 / 一次性夹具）。共享资源（同一库、同一出口 IP、同一账号配额）必须**显式登记**，并给出隔离手段（独立用户、独立库名 / Redis db index、夹具直连预置、用例内串行等待）。
2. **预算核对**：用例集启用前必须核对**前置预算**——「用例数 × 每用例消耗的受限配额」必须小于限流窗口（实测：注册 / 登录 10 次/分钟按 IP，被前序用例占满 → 429 退避重试每次 11–12s → 耗尽 30s 用例超时）。预算不足时改用**预置账号 / 独立配额**，**不得靠退避重试硬扛**。
3. **`retries` 必须为 0**：不得靠重试把不稳定掩盖成通过。确需开启时必须**单独登记**「靠重试通过」的用例，且该轮结论**不得作为回归基线**。
4. **不稳定判定（三类归因，顺序固定）**：① **环境未就绪**（签名与判据见 `docs/role-protocol.md` §5：`browserType.launch` / `ECONNREFUSED` / 页面无 `page-*` 锚点 / 前端未挂载）；② **用例隔离缺陷**（全量运行失败而隔离复跑通过，且失败签名为超时并指向共享配额或共享状态）；③ **产品缺陷**。判为 ② 的必须登记为 OBS（含根因 + 处置计划），**不得据此记产品缺陷**。
5. **隔离复跑的使用限制**：结论取自隔离复跑时，报告必须**显式声明**「本结论取自隔离复跑，本轮全量运行不稳定」；**同一签名跨 ≥2 轮复发 → 必须作为用例缺陷处置**（补隔离手段或登记为未闭环），**不得继续用「隔离复跑通过」放行**——否则不稳定会被永久洗成绿。

### 7.7 统计与概率类验收

概率 / 抽样 / 分位值类验收的判据不是"跑一遍看结果"，任何一项不满足都会让结论**不可复现或不可信**：

1. **阈值只取契约原值**，禁止放宽、禁止自定；样本量下限取契约值，契约未规定时由架构阶段给定并登记。
2. **判定对象在整个采样窗口内必须恒定**：候选集 / 库存 / 权重 / 开关不得在采样中途变化（实测：默认库存合计 263，1 万次抽样必然中途耗尽 → 候选集变化 → **前半与后半样本落入不同概率口径**）。夹具须钉住这些量，并在报告中**声明为非默认口径**。
3. **样本纯净性核验**：采样窗口内不得混入非本批流量，须以**请求计数对账**或等效手段证明（本仓库做法：实例日志 `responded` 行数 = 期望条数，异常 / 限流 / 4xx 逐类核对）。
4. **前置不可满足必须登记偏离**：如「固定随机源」在实现下不可构造（见 6.6 第 5 条**第 4 问**「随机源可控性」——**按名称引用，不按序号**，序号会随条款增删漂移），必须在**脚本头部与报告中**显式登记偏离，**且不得声称已满足该前置条件**。
5. **复位义务**：被改动的夹具必须在脚本 `finally` 内复位，并**实测确认**（记录复位后的实际值），不得只写"已复位"。
...
- **两类文本的处置方向相反，不得混用**：**配置文件与代码注释里的实然陈述必须随事实同步**——它们描述"现在是什么"，事实变了就必须改（实测：某配置文件把「本机仓库路径含 `&`」写成**具体事实**，仓库改名后该断言静默失效，而规范侧写的条件式表述仍然成立）；**已签署产物中的历史复述不回改**——那是"当时事实的真实记录"，回改会破坏取证价值（如历史轮次记录里的行号引用、当时的失败值）。判断口径：**能被后来者当作当前事实执行的（配置 / 注释 / 活的规范）→ 必须同步；被当作历史快照读的（已冻结产物的既有段落）→ 不回改、只能在新增段落里注明**
- **「活文档」是闭集，且它就是护栏的扫描面（本条与护栏 T30 同源）**：`CLAUDE.md`、`README.md`、`.claude/README.md`、`.claude/agents/**`、`.claude/commands/**`、`docs/artifacts.md`、`docs/development-spec.md`、`docs/role-protocol.md`、`docs/state-machine.md`、`docs/config-checklist.md`、`docs/error-codes.md`、`global/CLAUDE.md`，以及 `src/`、`tests/` 与 `tools/` 的代码与注释。**这些文本里不得出现「文件名:行号」型引用**；行号只允许作为**被叙述对象**出现（如「原以某行号为锚点，架构升版后该锚点已漂移」——描述漂移事实本身的句子），且该句必须同时给出稳定 ID。已签署产物（00–60）的既有段落按上条**不回改**，其行号引用的订正走各产物的**勘误块**（体例见 `docs/51-defects.md` 头部「证据引用勘误」）。**机检范围的边界必须显式声明**：护栏机检的形态是「文件名:行号」（含 `src/` / `tests/` / `tools/` 的代码文件）；**机检形态为封闭三型**：① `文件名:行号`（含 `src/` / `tests/` / `tools/` 的代码文件）；② `类型名.方法名:行号`（无扩展名的点号链，要求至少一段大写开头——小写点号链是数据键（`count: 3`）而非符号）；③ 同行已有文件引用时**反引号包裹的裸行号续写**（`:NNN`，即同一处定位引用的另一半：只删文件那半即逃过 ①）。**残余边界同样必须写出来**：单段符号名（`DrawService:149`）与 JS 对象键（`PASS: 0`）在文本上不可分辨、跨行续写（文件引用与裸行号不同行）不在机检面内——这两类只判形态必误报，属**人工复核面**；它们与前三型的违规性质相同（本条的禁止对象是「以行号定位」这件事，与是否落在机检面无关）。实测：`tests/e2e/api/stats-lib.mjs` 曾以「集成夹具方法:行号范围」的形式留着一处行号锚点，按「文件名:行号」口径扫描时全程看不见（本批已按 ② 清掉）；`.claude/commands/feature.md` 曾用三个裸行号指向 `docs/60-review.md` 的三处声明、三处**全部漂移**，而 ③ 落地前护栏同样看不见（本批已按 ③ 清掉，改指 §B/§F.3/§F.5 与 `60:OBS-28`）——**机检面比规则窄，就必须把边界写出来**，否则读者会把「护栏全 PASS」读成「没有行号引用」。**扫描面上的每一处豁免都必须登记在案**（锚句 + 理由），护栏会断言豁免锚句仍然存在——豁免是登记披露通道，不是静默放宽的通道。
...
- [ ] 新逻辑附带测试
- [ ] **修复类改动已做负向验证**（回退修复 → 断言必红 → 还原复绿），三步观测已记入 changelog（7.5 第 6 条）
- [ ] **新增 / 修改的用例自带隔离前提**（不依赖执行顺序、不共享限流窗口与可变夹具；`retries` 为 0）（7.6）
- [ ] **统计 / 概率类用例**已核对：阈值取自契约、判定对象全程恒定、样本纯净性对账、前置偏离已登记、夹具已复位并实测（7.7）
- [ ] 关键操作有审计日志
- [ ] 页面/接口满足性能预算
