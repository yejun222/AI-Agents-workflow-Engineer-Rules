...
| OBS-03 | **抽奖同键异体的 409 分支在契约内不可达** | D-03 规定抽奖请求体恒为 `{}`，其规范化哈希基于空对象常量；额外字段不参与哈希 → 「请求体不同」不可构造，服务端按幂等**正常重放**（实测 HTTP 200 / `code=0`，仅扣 1 次、流水行 1 条）。核心不变量（不产生第二次副作用）满足；**阻断标注（2026-09-18，默认档）**：**可测性缺口**——TC-53 契约分支（同键异体 → 409）在本契约下不可构造、该用例判据**未按设计达成**（§5 已记「未达断言口径」）→ 牵动「覆盖是否达标」的成立（D1 分子的「明确结论」口径）→ 拿不准按**默认档**登记；是否放宽口径须主对话 / 用户裁决（见 `docs/52-qa-report.md` §4 新增待确认项）。 | TC-53 | 阻断 |
...
| OBS-05 | **应用日志未落盘为文件** | Serilog 仅 `WriteTo: Console`，无法离线检索日志内容；TC-81 的「日志不含明文密码 / token」改为以 `AuditLog` 表 + `DrawRequest.RequestHash` 为可检索面验证（均无命中），并复以 `User.PasswordHash` 为 BCrypt（TC-82）交叉印证；**阻断标注（2026-09-18，默认档）**：观察面 = **证据可检索性**（日志未落盘）→ 属**证据类**，按默认档登记；TC-81 改道 `AuditLog` / `DrawRequest.RequestHash` / `User.PasswordHash` 不能消除「判据面受限」本身。 | TC-81 | 阻断 |
| OBS-06 | **服务端未提供时钟注入通道** | 跨日重置与时间口径只能以「按 `DrawDate` 自然日分桶」的夹具等价验证（TC-57/58/59），23:59:59 / 00:00:00 的**逐秒边界**无法执行；**阻断标注（2026-09-18，默认档）**：属**环境 / 工具链类**（无时钟注入 → 23:59:59 / 00:00:00 逐秒边界不可构造），按默认档登记；TC-57/58/59 系夹具等价执行。 | TC-57、TC-58、TC-59 | 阻断 |
| OBS-08 | **Playwright 的 `outputDir` 每次运行前会被自动清空**（Playwright 默认行为，非人工删除） | `tests/e2e/.artifacts/` 在每次 `node node_modules/@playwright/test/cli.js test` 启动时被清空，故**上一轮（14:35–14:38）4 个失败截图 / trace 目录已被后续复跑覆盖，磁盘上不再存在**。可持久化的证据改为落在 `outputDir` 之外的 `tests/e2e/logs/*.log`（重定向全量标准输出）。当前可核验证据：`tests/e2e/logs/full-run-20260917-144348.log`（47 passed / 3 failed）+ `tests/e2e/.artifacts/` 下本轮 3 个失败目录；**阻断标注（2026-09-18，默认档）**：属**工具行为 + 证据类**（`outputDir` 每次运行前自动清空），按默认档登记；实测（2026-09-18）`tests/e2e/.artifacts/` 目录**已不存在**，本行所引「本轮 3 个失败目录」当前不可核验（证据强度按**旁证**计；见头部「引用勘误」行）。 | 全体 E2E | 阻断 |
