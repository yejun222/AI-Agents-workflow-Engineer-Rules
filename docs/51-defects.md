...
| 产物 | 缺陷报告（defects） |
| 版本 | v1 |
| 冻结时间 | 2026-09-17T18:31:00+08:00（**R7 回写的实测读数** `18:31:01` 向下取整到分钟——即该次落盘本身，按 `docs/artifacts.md` §4「冻结时间的判据」即「末次实际落盘时刻」）（**2026-09-18 勘误**：① 本条原附的推导**不成立**：原文所称「实际落盘时刻 `17:18:40`」向下取整应为 `17:18`，而取值 `18:31` 实为 R7 回写**同批同源**的读数（与 `docs/52-qa-report.md` 冻结时间行同值）—— 原句把两个不同来源的时刻混写成了同一次推导；**取值本身无误，只订正推导与证据来源**。② 另登记本条订正**之后**的一次落盘：「OBS 编号勘误」块自标日期 `2026-09-17`，而承载它的提交为 **`2026-09-18 03:21:59`**（`7c163f7`）—— 该块的**实际落盘日无法由证据确证**（可能写于 09-17 夜间、09-18 凌晨才提交；同一提交内含 09-18 02:46–03:02 产生的日志），按 §4「落盘证据只认提交时刻」只记其**落盘上界** = `2026-09-18 03:21:59`，**不推定**。该块属**引用勘误**（不涉契约内容），按 §4 第 5 条**不改版本**，故此处**只登记、不升版**）；④ 另登记本条订正**之后**的又一次落盘（2026-09-18）：新增「执行侧补验标签勘误」行（`TC-13b` / `TC-39b` 登记与订正）+ `TC-13` 行的引用订正（行号引用改为稳定 ID）。落盘时刻实测 = `2026-09-18T11:33`（`date` 实测输出，向下取整到分钟）；**承载提交尚未产生**（本角色不提交代码，须由主对话 wip 提交承载）→ 按 §4 只登记落盘日 `2026-09-18`、**不推定提交 hash**。本批属**引用勘误**，按 §4 第 5 条**不改版本**。）；**⑤ 2026-09-20 落盘**：§3 观察项表新增 `51:OBS-12` 行 + 头部新增两行（台账回写 / 新增观察项登记 + `60-review` 台账回写轮同步与 `60:OBS-32` 扇出复核）。落盘时刻实测 = `2026-09-20T11:00:32`（python datetime.now()，本机 `date` 不可用）；**承载提交尚未产生**（本角色不提交代码）→ 按 §4 只登记落盘日 `2026-09-20`、**不推定提交 hash**。本批属**台账类回写**（新增观察项登记 + 状态同步复核），按 §4 第 5 条**不升版**。**⑥ 2026-09-20 Wave D 落盘**：`51:OBS-12` 闭环回写（状态置 `已闭环`、末列「阻断」重标 `非阻断`）+ 新增 `51:OBS-13` / `51:OBS-14` 两行 + 头部新增两行（闭环 / 新增观察项登记行，`tests/integration` 夹具变更的 §5 扇出复核行）。落盘时刻实测 = `2026-09-20T11:44:49`（python datetime.now()，本机 `date` 不可用）；**承载提交尚未产生**（本角色不提交代码）→ 按 §4 只登记落盘日 `2026-09-20`、**不推定提交 hash**。本批属**台账类回写（状态回写 / 阻断标注 / 新增观察项登记）+ 复核结论登记**（任何 BUG 状态与判定结论均未变）→ 按 §4 第 5 条**不升版**。**（2026-09-20 取值订正）**：⑥ 原记落盘时刻 `2026-09-20T11:43:49` 系批内较早一次读数、比该批末次实际写入早 1 分钟；按 `docs/artifacts.md` §4「取值 = 该版本末次实际落盘时刻」**订正为 `2026-09-20T11:44:49`**（证据链 = **本角色自报 + 主对话复核文件 mtime**；注意：按 §4「落盘证据只认 git 提交时刻」，**文件 mtime 不作落盘证据** —— 本项仅为取值订正、不主张落盘证据强度；**承载提交 hash + 提交时间待回填**，提交后由主对话调度本角色回填）。**⑦ 2026-09-20 B 批落盘**：§3 观察项表 6 条阻断型行（`51:OBS-03` / `51:OBS-05` / `51:OBS-06` / `51:OBS-07` / `51:OBS-08` / `51:OBS-11`）描述列末尾补对侧编号标注 + 头部新增一行（本批台账回写 / 取值订正登记行）。落盘时刻实测 = `2026-09-20T12:03:14`（python datetime.now()，本机 `date` 不可用）；**承载提交尚未产生**（本角色不提交代码）→ 按 §4 只登记落盘日 `2026-09-20`、**不推定提交 hash**。本批属**台账类回写（阻断型 OBS 行对侧编号标注）+ 头部元信息取值订正**（不改任何阻断列取值、不改任何判定结论）→ 按 §4 第 5 条**不升版**。 |
