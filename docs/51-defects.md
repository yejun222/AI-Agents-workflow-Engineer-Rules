...
| 阻断标注（2026-09-18） | §3 观察项表**新增末列「阻断 / 非阻断」**（结构化取值；判别口径 = `docs/artifacts.md` §3.1 / `docs/state-machine.md` D2 第 2 项：**环境 / 证据 / 工具链类默认按「阻断」登记**，仅在「该观察项不影响任何结论的成立」时才标 `非阻断`，逐条理由写在本表各行「判定依据」列内）。本文件序列 9 条**实测取值：阻断 6**（OBS-03 / OBS-05 / OBS-06 / OBS-07 / OBS-08 / OBS-11）、**非阻断 3**（OBS-01 / OBS-02 / OBS-04）；同批 `docs/52-qa-report.md` §3 同源登记（其序列 11 条：阻断 6 / 非阻断 5）。属 §4 第三类**台账类回写（阻断标注）→ 按 §4「不升版」，仅登记**（改动哪一类 / 何时 / 依据哪条规则 = 本条）；**OBS 编号、编号勘误块与历史行文一律不回改**，本行**不改变任何 BUG 状态与结论**。 |
...
| OBS-03 | **抽奖同键异体的 409 分支在契约内不可达**。对侧同一事实：↔ 52:OBS-03 | D-03 规定抽奖请求体恒为 `{}`，其规范化哈希基于空对象常量；额外字段不参与哈希 → 「请求体不同」不可构造，服务端按幂等**正常重放**（实测 HTTP 200 / `code=0`，仅扣 1 次、流水行 1 条）。核心不变量（不产生第二次副作用）满足；**阻断标注（2026-09-18，默认档）**：**可测性缺口**——TC-53 契约分支（同键异体 → 409）在本契约下不可构造、该用例判据**未按设计达成**（§5 已记「未达断言口径」）→ 牵动「覆盖是否达标」的成立（D1 分子的「明确结论」口径）→ 拿不准按**默认档**登记；是否放宽口径须主对话 / 用户裁决（见 `docs/52-qa-report.md` §4 新增待确认项）。 | TC-53 | 阻断 |
