...
> ⚠️ **仓库绝对路径含 `#` 时，前端单测会全量失败**，且**与 `npm run` 无关**——Vitest/Vite 自身的路径解析被 `#` 打断，报 `Cannot find module '/src/...'`。实测对照：同一份代码在无 `#` 的路径下全通过。处置：移到不含 `#` 的路径，或在无 `#` 的镜像目录中运行并登记其绝对路径作为取证来源。
...
> ⚠️ **长驻开发实例会锁死 `bin/` 输出目录**（报 `MSB3027` / `MSB3021`，且产物停留在改动前版本）：构建时一律写临时输出目录 `-p:OutputPath=<临时目录>`，并**登记「`bin/` 内产物是改动前版本」**、声明哪些验证因此不可信。
...
- 重试判定必须在事务边界统一分类，禁止逐写入点手写 catch（每新增一个写路径就会静默失效一次）；客户端命令超时必须 ≥ 服务端同类超时（配反会让"为服务端超时写的分支"永不可达），且任何为某种超时写的分支都必须验证真的可达（详见全量规范 4.4）。
...
- 契约能用键值表就别用散文（同一事实单点定义，其余处引用 ID）；每个分支都必须可构造；401/403/429 等通用错误响应体形态必须一次写死（详见全量规范 6.6）。
- 引用一律用稳定 ID，禁止用行号（行号必然漂移）；引用前实测，声称"实测"的数字必须真的来自实测（详见全量规范 13.4）。
...
- 安全敏感配置只允许一个取值来源，且必须启动期 fail-fast（禁止多点各自兜底）；配置缺失的表象是"接口 500 + 日志 0 字节"（详见全量规范 8.7）。
...
- 断言必须来自契约，禁止把当前行为固化成期望（实测出现过用例把缺陷写成期望，于是修复前套件全绿）；修改断言只允许加强，禁止为求全绿放宽；新增断言必须说明它在什么情况下会失败（详见全量规范 7.5）。
- 组件渲染测试（模板层）必须有：模板层的条件渲染分支、兜底文案与展示格式化必须有组件渲染测试（`jsdom` + `@vue/test-utils` 基建已具备，不得以「无基建」为由豁免，详见全量规范 7.2）。
...
- [ ] 新逻辑附带测试，且已本地跑通
- [ ] 新增/修改的测试断言已说明辨别力依据（只加强不放宽，未被"为求全绿"调整）
- [ ] 引用上游契约与产物一律用稳定 ID（未用行号），且引用前已实测
- [ ] 关键操作有审计日志
